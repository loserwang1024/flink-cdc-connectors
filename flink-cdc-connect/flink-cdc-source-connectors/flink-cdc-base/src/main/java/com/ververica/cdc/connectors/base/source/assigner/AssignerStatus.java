 ...
 *              |
 *              ↓
 *     NEWLY_ADDED_ASSIGNING ---onFinish()--→ NEWLY_ADDED_ASSIGNING_SNAPSHOT_FINISHED---onStreamSplitUpdated()---> NEWLY_ADDED_ASSIGNING_FINISHED(end)
 *              ↑                                                                                                               |
 *              |                                                                                                               |
 *              |--------------- startAssignNewlyTables() //found newly added tables, assign newly added tables ----------------|
 * </pre>
...
public enum AssignerStatus {
    ...
    NEWLY_ADDED_ASSIGNING_SNAPSHOT_FINISHED(3) {
        ...
        @Override
        public AssignerStatus onStreamSplitUpdated() {
            LOG.info(
    ...
    public AssignerStatus onStreamSplitUpdated() {
        throw new IllegalStateException(
                format(
                        "Invalid call, assigner under %s state can not call onStreamSplitUpdated()",
                        fromStatusCode(this.getStatusCode())));
    }
