...
public class FlussSinkWriter<InputT> implements SinkWriter<InputT> {
        ...
        try {
            ...
            for (FlussRowWithOp rowWithOp : flussEvent.getRowWithOps()) {
                ...
                CompletableFuture<?> writeFuture =
                        multiTableWriter.write(
                                MultiTableWriteRecord.of(
                                        tablePath, toChangeType(opType, tablePath), row, schemaId));
    ...
    private static ChangeType toChangeType(FlussOperationType opType, TablePath tablePath) {
        switch (opType) {
            case APPEND:
